...
async function fetchRoute(coords) {
  ...
  try {
    console.log("Fetching route for coordinates:", coords);
...
export default function RoutesMap({
  ...
  useEffect(() => {
    ...
    async function buildRoutes() {
      ...
      if (!start) {
        setError("No starting location provided");
        setLoading(false);
        return;
      }
      ...
      try {
            ...
            passengers.map(async (passenger) => {
              try {
                ...
                if (!pickupCoords || !dropCoords) {
                  console.warn(
                    `Skipping passenger ${passenger.id} due to geocoding failure`
                  );
                  return null;
                }
                ...
                const routeCoords = [
                  [
                    driverLocationCoords.longitude,
                    driverLocationCoords.latitude,
                  ],
                ...
                if (!route) {
                  console.warn(`No route found for passenger ${passenger.id}`);
                  return null;
                }
                ...
                console.error(
                  `Error processing passenger ${passenger.id}:`,
                  err
                );
                return null;
              }
        ...
        if (isMounted) {
          setError(
            err.message ||
              "Failed to load routes. Please check addresses and try again."
          );
        }
      } finally {
        if (isMounted) {
  ...
  }, [driverLocation, passengers, mode, startAddress]);

  if (loading) return <div className="p-4 text-center">Loading routes...</div>;
  if (error) return <div className="p-4 text-center text-red-500">{error}</div>;
  if (!driverCoords)
    return <div className="p-4 text-center">No driver location available</div>;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Driver Marker */}
        <Marker position={mapCenter} icon={DRIVER_ICON}>
          <Popup>Driver Location: {driverLocation}</Popup>
        </Marker>

        {/* Passenger Routes */}
        {routes.map((route) => {
          if (!route.routeData?.features?.[0]?.geometry?.coordinates) {
            console.warn("No coordinates for route:", route.id);
            return null;
          }
          ...
          return (
            <React.Fragment key={route.id}>
              <Marker
                position={[
                  route.pickupCoords.latitude,
                  route.pickupCoords.longitude,
                ]}
                icon={PICKUP_ICON}
              >
                <Popup>
                  <div>
                    ...
                    <br />
                    Pickup: {route.pickupCoords.address || route.pickup}
                    <br />
              ...
              <Marker
                position={[
                  route.dropCoords.latitude,
                  route.dropCoords.longitude,
                ]}
                icon={DROP_ICON}
              >
                <Popup>
                  <div>
                    <strong>{route.name}</strong>
                    <br />
                    Drop: {route.dropCoords.address || route.drop}
                    <br />
              ...
              <Polyline positions={line} color={color} weight={3} opacity={0.7}>
